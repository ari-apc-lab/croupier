...
if [ -n "$pkey" ]; then
  ssh -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -i $pkey $user@$host 'rm -rf permedcoe_apps/drug-synergies/'
fi
if [ -n "$password" ]; then
  sshpass -p "$password" ssh -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -i $pkey $user@$host 'rm -rf permedcoe_apps/drug-synergies/'
fi
